...
import { CloudAppSettingsService, AlertService, FormGroupUtil, CloudAppStoreService, CloudAppConfigService } from '@exlibris/exl-cloudapp-angular-lib';
  ...
  load() {
    this.configService.getAsFormGroup().subscribe( config => {
      if (!isEmptyObject(config.value)) {
        (config.get('profiles') as FormArray).controls.forEach((profile: FormGroup)=>{
...
export class ConfigGuard implements CanDeactivate<ConfigComponent> {
  constructor(
    private dialog: DialogService,
  ) {}

  canDeactivate(component: ConfigComponent): Observable<boolean> {
